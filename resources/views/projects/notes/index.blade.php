                ...
                @foreach($notes as $note)
                    <div class="rounded">
                        <div class="w-full h-64 flex flex-col justify-between dark:bg-gray-800 bg-white dark:border-gray-700 rounded border border-gray-200 py-5 px-4">
                            <div>
                                <h4 class="text-gray-800 dark:text-gray-100 font-bold mb-3">{{ \App\Models\User::find($note->user_id)->name }}</h4>
                            ...
                            <div>
                                <div class="flex items-center justify-between text-gray-800 dark:text-gray-100">
                                    <p class="text-sm">{{ \Carbon\Carbon::parse($note->created_at)->format('d. M. Y') }}</p>
                                    <div class="flex">
                                    ...
                                    @endif
                                    <a href="{{ route('project.notes.show', $note->id) }}" class="text-purple-500 hover:text-purple-600 flex items-center justify-center duration-300 mr-4 last:mr-0" aria-label="edit note" role="button">
                                    ...
                                    @if($note->user_id === auth()->id() or is_owner())
                                        <a href="{{ route('project.notes.show', $note->id) }}" class="text-red-500 hover:text-red-600 flex items-center justify-center duration-300 mr-4 last:mr-0" aria-label="edit note" role="button">
