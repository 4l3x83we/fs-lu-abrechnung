...
@else
    <div class="relative">
        <input value="{{ old($id ?? '') }}" type="{{ ($type ?? 'text') }}" id="{{ ($id ?? '') }}" name="{{ ($id ?? '') }}" {{ $attributes->merge(['class' => 'shadow-sm bg-gray-50 border border-gray-300 text-gray-900 placeholder-gray-700 text-xs rounded focus:ring-gray-500 focus:border-gray-500 block w-full pr-12 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500', 'placeholder' => $text ?? '']) }} />
        <div class="absolute text-xs inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
            {{ $icon ?? '' }}
        </div>
    </div>
