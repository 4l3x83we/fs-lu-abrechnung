@error($id ?? '')
    ...
    <select  id="{{ $id ?? '' }}" name="{{ $id ?? '' }}" {{ $attributes->merge(['class' => 'shadow-sm bg-red-50 border border-red-500 text-red-900 placeholder-red-700 text-xs rounded focus:ring-red-500 dark:bg-gray-700 focus:border-red-500 block w-full p-2 dark:text-red-600 dark:placeholder-red-500 dark:border-red-500', 'placeholder' => $text ?? '']) }} >
        {{ $slot }}
    </select>
