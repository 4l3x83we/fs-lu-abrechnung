...
@else
    <div class="sm:flex sm:items-center">
        ...
        <div class="sm:w-2/3">
            <input {{ $attributes->merge(['type' => $type ?? 'text', 'wire:model' => $id ?? '', 'id' => $id ?? '', 'value' => old($id ?? ''), 'name' => $id ?? '', 'class' => 'shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded focus:ring-gray-500 focus:border-gray-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500', 'placeholder' => $text ?? '']) }} />
        </div>
    </div>
    @enderror
